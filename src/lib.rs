...
pub fn display_counters() {
    ...
    println!("Origin check errors: {}", COUNTER_ORIGIN_CHECK_ERROR.load(Ordering::Relaxed));
    println!("Successors not snapshots: {}", COUNTER_NOT_SNAPSHOT.load(Ordering::Relaxed));
    println!("Snapshots without committer timestamp: {}", COUNTER_NO_COMMITTER_TIMESTAMP.load(Ordering::Relaxed));
    println!("Origins with insufficient snapshots (<2): {}", COUNTER_INSUFFICIENT_SNAPSHOTS.load(Ordering::Relaxed));
    println!("Invalid UTF-8 branch names: {}", COUNTER_INVALID_UTF8_BRANCH_NAME.load(Ordering::Relaxed));
    println!("Branches not containing '/tags/': {}", COUNTER_NOT_TAG_BRANCH.load(Ordering::Relaxed));
    println!("Tag successors not releases: {}", COUNTER_NOT_RELEASE.load(Ordering::Relaxed));
    println!("Releases with invalid revision count (!=1): {}", COUNTER_INVALID_REVS_COUNT.load(Ordering::Relaxed));
    println!("======================================\n");
}
...
pub fn tags_check_full<G: SwhFullGraph + Sync>(graph: &G) -> Result<()> {
    (0..graph.num_nodes())
        ...
        .for_each(|origin| match tags_check_origin(origin, graph) {
            Ok(_) => (),
            Err(_) => {
                COUNTER_ORIGIN_CHECK_ERROR.fetch_add(1, Ordering::Relaxed);
            }
        });
    Ok(())
}

fn tags_check_origin<G: SwhFullGraph>(origin: usize, graph: &G) -> Result<()> {
    ...
    let mut current_snapshot = snapshots_queue.pop_front().unwrap();
    ...
    while let Some(next_snapshot) = snapshots_queue.pop_front() {
        ...
        //compute_inconsistencies(&mut inconsistencies, &current_tags, &next_tags, graph);
